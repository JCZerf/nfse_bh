...
- `GET /health`: status local da API, sem chamada externa.
- `GET /health/deep`: status da API + conectividade real com o BHISS. Requer `X-API-Key`.
- `GET /metrics`: métricas no formato Prometheus. Requer `X-API-Key`.
