...
def test_requests_beyond_limit_are_rejected():
    ...
    statuses = [client.get("/metrics", headers={"X-API-Key": api_key}).status_code for _ in range(35)]
...
def test_rate_limit_is_isolated_per_key():
    ...
    for _ in range(31):
        client.get("/metrics", headers={"X-API-Key": key_a})

    response_a = client.get("/metrics", headers={"X-API-Key": key_a})
    response_b = client.get("/metrics", headers={"X-API-Key": key_b})
